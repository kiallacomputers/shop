...
export default defineEventHandler(async (event) => {
  ...
  const name=String(body?.name||"").trim(), productCode=String(body?.product_code||"").trim(); const price=body?.price==null||body?.price===""?null:Number(body.price), oldPrice=body?.old_price==null||body?.old_price===""?null:Number(body.old_price), stock=Number(body?.stock);
  if(!name||!productCode) throw createError({statusCode:400,statusMessage:"Variant name and product code are required"});
  if((price!==null&&(!Number.isFinite(price)||price<0))||!Number.isInteger(stock)||stock<0) throw createError({statusCode:400,statusMessage:"Enter a valid price and stock level"});
