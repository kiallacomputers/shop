...
import { assertAccountingDateOpen } from "~~/server/utils/accountingPeriods";
...
export async function createPostedJournal(input:{journal_date?:string,reference?:string,description:string,source_type?:string,source_id?:string,posted_by?:string,lines:any[]}) {
  ...
  const journalDate=input.journal_date||new Date().toISOString().slice(0,10);
  await assertAccountingDateOpen(journalDate);
