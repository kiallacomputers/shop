...
<section class="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-6" id="account-overview" v-show="activeAccountSection === 'account-overview'">
      ...
      <section class="kc-panel p-6 mb-8" id="account-accounting" v-show="activeAccountSection === 'account-accounting'">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div><h2 class="text-xl font-bold text-slate-900">Invoices & Statement</h2><p class="mt-1 text-sm text-slate-500">Your accounting history from Kialla Computers.</p><a v-if="customerAccountingLinked" href="/api/account/accounting/statement.pdf" class="mt-2 inline-block text-sm font-bold text-blue-600 hover:text-blue-700">Download PDF Statement →</a></div>
        ...
        <div v-else-if="customerInvoices.length" class="mt-5 overflow-x-auto rounded-xl border border-slate-200">
          <table class="min-w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody class="divide-y divide-slate-100 bg-white"><template v-for="invoice in customerInvoices" :key="invoice.id"><tr><td class="px-4 py-3 font-bold text-slate-900">{{ invoice.invoice_number || `INV-${invoice.id}` }}</td><td class="px-4 py-3 text-slate-600">{{ formatDate(invoice.invoice_date) }}</td><td class="px-4 py-3"><span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold capitalize text-slate-700">{{ invoice.status || 'unpaid' }}</span></td><td class="px-4 py-3 text-right font-semibold">{{ currency(invoice.total) }}</td><td class="px-4 py-3 text-right text-emerald-700">{{ currency(invoice.paid_amount) }}</td><td class="px-4 py-3 text-right font-bold" :class="invoiceBalance(invoice) > 0 ? 'text-amber-700' : 'text-emerald-700'">{{ currency(invoiceBalance(invoice)) }}</td></tr><tr v-for="payment in invoice.accounting_customer_payments || []" :key="`payment-${payment.id}`" class="bg-emerald-50/40"><td class="px-4 py-2 pl-8 text-xs font-bold text-emerald-700">↳ Payment</td><td class="px-4 py-2 text-xs text-slate-600">{{ formatDate(payment.payment_date) }}</td><td class="px-4 py-2 text-xs text-slate-600">{{ payment.payment_method }}<span v-if="payment.reference"> · {{ payment.reference }}</span></td><td class="px-4 py-2"></td><td class="px-4 py-2 text-right text-xs font-bold text-emerald-700">{{ currency(payment.amount) }}</td><td class="px-4 py-2"></td></tr></template></tbody>
          </table>
        </div>
