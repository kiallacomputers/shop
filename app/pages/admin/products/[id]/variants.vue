      ...
      <article v-for="variant in variants" :key="variant._key" class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div class="mt-4 rounded-lg bg-slate-50 p-4">
          ...
          <select
            ...
            @change="setVariantMainImage(variant, ($event.target as HTMLSelectElement).value)"
          >
            <option value="">Use default product image</option>
            <option v-for="(image,index) in productImages" :key="image" :value="image">
          ...
          <div class="mt-3 flex items-start gap-4">
            <div class="h-28 w-28 shrink-0 overflow-hidden rounded-xl border bg-white">
              <img
                v-if="variant.images?.[0] || productImages[0]"
                :src="variant.images?.[0] || productImages[0]"
                class="h-full w-full object-contain p-2"
              />
            </div>
            <div class="min-w-0 flex-1">
              <p class="text-sm font-bold text-slate-800">{{ variant.images?.[0] ? 'Selected variant image' : 'Default product image' }}</p>
              <p class="mt-1 text-xs text-slate-500">The complete main product gallery remains available on the storefront.</p>
            </div>
          </div>
          ...
          <div v-if="productImages.length" class="mt-4">
            ...
            <div class="flex flex-wrap gap-2">
              <button
                ...
                :key="image"
                type="button"
                class="relative h-16 w-16 overflow-hidden rounded-lg border-2 bg-white transition hover:border-blue-500"
                :class="variant.images?.[0] === image ? 'border-blue-600 ring-2 ring-blue-100' : 'border-slate-200'"
                :title="`Use Product Image ${index + 1}`"
                @click="setVariantMainImage(variant,image)"
              >
                <img :src="image" class="h-full w-full object-contain p-1" />
                <span v-if="variant.images?.[0] === image" class="absolute inset-x-0 bottom-0 bg-blue-600 py-0.5 text-[9px] font-bold text-white">SELECTED</span>
              </button>
            </div>
          </div>

          <p v-else class="mt-3 rounded-lg bg-amber-50 p-3 text-xs font-semibold text-amber-800">
            Add images to the main product first, then return here to assign one.
          </p>
        </div>
...
const setVariantMainImage=(variant:any,image:string)=>{
  const selected=String(image||'').trim();
  variant.images=selected?[selected]:[];
};
