  ...
  <main class="admin-content">
    ...
    <section class="admin-panel mt-5 p-5">
      ...
      <div class="mt-4 flex flex-wrap gap-3">
        <NuxtLink class="tool-icon tool-blue" to="/admin/accounting/accounts" title="Chart of Accounts" aria-label="Chart of Accounts">
          <ToolIcon path="M4 5h16v14H4V5Zm4 4h8M8 13h3m2 0h3M8 17h3m2 0h3" />
        </NuxtLink>
        <NuxtLink class="tool-icon tool-violet" to="/admin/accounting/journals" title="General Journal" aria-label="General Journal">
          <ToolIcon path="M6 3h9l3 3v15H6V3Zm3 7h6m-6 4h6m-6 4h4" />
        </NuxtLink>
        <NuxtLink class="tool-icon tool-green" to="/admin/accounting/invoices" title="Sales & Invoices" aria-label="Sales & Invoices">
          <ToolIcon path="M6 3h12v18H6V3Zm3 5h6m-6 4h6m-6 4h3" />
        </NuxtLink>
        <NuxtLink class="tool-icon tool-cyan" to="/admin/accounting/receivables" title="Accounts Receivable" aria-label="Accounts Receivable">
          <ToolIcon path="M4 6h16v12H4V6Zm3 4h10M8 14h4m5-2 2 2-2 2" />
        </NuxtLink>
        <NuxtLink class="tool-icon tool-orange" to="/admin/accounting/purchases" title="Suppliers & Purchases" aria-label="Suppliers & Purchases">
          <ToolIcon path="M3 7h18l-2 12H5L3 7Zm4 0 2-3h6l2 3M8 12h8" />
        </NuxtLink>
        <NuxtLink class="tool-icon tool-red" to="/admin/accounting/payables" title="Accounts Payable" aria-label="Accounts Payable">
          <ToolIcon path="M4 6h16v12H4V6Zm3 4h10m-7 4h4m-5-2-2 2 2 2" />
        </NuxtLink>
      </div>
    </section>
...
const ToolIcon = defineComponent({
  ...
  setup(props) {
    return () => h("svg", { viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", "stroke-width": "2", class: "h-8 w-8", "aria-hidden": "true" }, [h("path", { "stroke-linecap": "round", "stroke-linejoin": "round", d: props.path })]);
  },
});
async function load() {
  err.value = "";
...
.tool-icon { @apply flex h-16 w-16 items-center justify-center rounded-2xl border shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2; }
.tool-blue { @apply border-blue-200 bg-blue-100 text-blue-700 hover:border-blue-300 hover:bg-blue-200 focus:ring-blue-400; }
.tool-violet { @apply border-violet-200 bg-violet-100 text-violet-700 hover:border-violet-300 hover:bg-violet-200 focus:ring-violet-400; }
.tool-green { @apply border-emerald-200 bg-emerald-100 text-emerald-700 hover:border-emerald-300 hover:bg-emerald-200 focus:ring-emerald-400; }
.tool-cyan { @apply border-cyan-200 bg-cyan-100 text-cyan-700 hover:border-cyan-300 hover:bg-cyan-200 focus:ring-cyan-400; }
.tool-orange { @apply border-orange-200 bg-orange-100 text-orange-700 hover:border-orange-300 hover:bg-orange-200 focus:ring-orange-400; }
.tool-red { @apply border-red-200 bg-red-100 text-red-700 hover:border-red-300 hover:bg-red-200 focus:ring-red-400; }
</style>
