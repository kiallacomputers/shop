    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
      ...
      <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p class="mt-2 text-3xl font-bold text-slate-900">{{ accounts.length }}</p>
        </div>
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p class="mt-2 text-3xl font-bold text-emerald-600">{{ customerCount }}</p>
        </div>
      </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[950px] table-fixed text-left text-sm">
            <thead class="border-b border-slate-200 bg-slate-50">
              <tr>
                <th class="w-[25%] px-4 py-3 text-xs font-bold uppercase tracking-wide text-slate-500">Account</th>
                <th class="w-[25%] px-4 py-3 text-xs font-bold uppercase tracking-wide text-slate-500">Email</th>
                <th class="w-[15%] px-4 py-3 text-xs font-bold uppercase tracking-wide text-slate-500">Joined</th>
                <th class="w-[15%] px-4 py-3 text-xs font-bold uppercase tracking-wide text-slate-500">Last Sign In</th>
                <th class="w-[10%] px-4 py-3 text-center text-xs font-bold uppercase tracking-wide text-slate-500">Access</th>
                <th class="w-[10%] px-4 py-3 text-right text-xs font-bold uppercase tracking-wide text-slate-500">Action</th>
              </tr>
            </thead>
              ...
              >
                <td class="px-4 py-4">
                  <p class="truncate font-semibold text-slate-900">
                    {{ account.display_name || account.email || "User" }}
                  </p>
                  <p v-if="account.is_current_user" class="mt-1 text-xs font-semibold text-blue-600">
                    Your account
                  </p>
                </td>

                <td class="px-4 py-4">
                  <p class="truncate text-slate-700">{{ account.email || "No email" }}</p>
                </td>
                ...
                <td class="px-4 py-4 text-center">
                  <span
                    ...
                    :class="account.is_admin ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-600'"
                  >
                ...
                <td class="px-4 py-4 text-right">
                  ...
                  >
                    {{ changingId === account.id ? "Saving..." : "Make Admin" }}
                  </button>
                  ...
                  <button
                    ...
                    type="button"
                    :disabled="changingId === account.id"
                    class="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-50 transition"
                    @click="setAdmin(account, false)"
                  >
                    {{ changingId === account.id ? "Demoting..." : "Demote Admin" }}
                  </button>

                  <span
                    v-else
                    class="text-xs font-semibold text-slate-400"
                    title="You cannot demote your own administrator account."
                  >
                    Protected
                  </span>
                </td>
...
const filteredAccounts = computed(() => {
  ...
  return accounts.value.filter((account) => {
    ...
    if (roleFilter.value === "admin" && !account.is_admin) return false;
    if (roleFilter.value === "customer" && account.is_admin) return false;
...
const adminCount = computed(
  () => accounts.value.filter((account) => account.is_admin).length,
);
...
const loadAccounts = async () => {
  ...
  try {
    accounts.value =
      (await adminFetch<Account[]>("/api/admin/accounts")) || [];
  } catch (error: any) {
    errorMessage.value =
...
const setAdmin = async (account: Account, makeAdmin: boolean) => {
  ...
  try {
    await adminFetch(`/api/admin/accounts/${account.id}`, {
      method: "PUT",
      body: {
        isAdmin: makeAdmin,
      },
    ...
    successMessage.value = makeAdmin
      ? `${account.display_name || account.email} has been promoted to administrator.`
      : `${account.display_name || account.email} has been demoted from administrator.`;
  } catch (error: any) {
    errorMessage.value =
      error?.data?.statusMessage ||
      error?.statusMessage ||
